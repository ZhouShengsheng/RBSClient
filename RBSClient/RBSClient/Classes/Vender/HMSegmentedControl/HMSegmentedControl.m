...
- (void)commonInit {
    ...
    // Enable badge to draw out of border.
    self.clipsToBounds = NO;
    self.scrollView.clipsToBounds = NO;
    
    // Init badge controllers.
    _badgeControllers = [NSMutableArray array];
...
- (void)setSectionTitles:(NSArray *)sectionTitles {
    ...
    // Init text badget controllers.
    [self.badgeControllers removeAllObjects];
    NSUInteger count = _sectionTitles.count;
    for (NSUInteger i = 0; i < count; i++) {
        SSBadgeController *badgeController =
        [[SSBadgeController alloc] init];
        [self.badgeControllers addObject:badgeController];
    }
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            // Add text layer.
            SSBadgeController *badgeController = self.badgeControllers[idx];
            badgeController.layer = titleLayer;
            // Draw badge.
            [badgeController drawBadge];
            
            // Vertical Divider
            if (self.isVerticalDividerEnabled && idx > 0) {
                CALayer *verticalDividerLayer = [CALayer layer];
                //verticalDividerLayer.frame = rectDiv;
                verticalDividerLayer.frame = CGRectMake(verticalDividerLayer.frame.origin.x,
                                                        verticalDividerLayer.frame.origin.y + rectDiv.size.height*0.25,
                                                        verticalDividerLayer.frame.size.width,
                                                        verticalDividerLayer.frame.size.height*0.5);
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(self.bounds, touchLocation)) {
        ...
        if (self.shouldReportEveryTouch) {
            if (self.isTouchEnabled)
                [self setSelectedSegmentIndex:segment animated:self.shouldAnimateUserSelection notify:YES];
        } else if (segment != self.selectedSegmentIndex && segment < sectionsCount) {
