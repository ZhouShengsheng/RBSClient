...
+ (BOOL)hasViewWithParentView:(UIView*)viewController{
    if ([self viewWithParentView:viewController]) {
        return YES;
    }
    return NO;
}
...
+ (MozTopAlertView*)viewWithParentView:(UIView*)viewController{
    NSArray *array = [viewController subviews];
...
+ (MozTopAlertView*)viewWithParentView:(UIView*)viewController cur:(UIView*)cur{
    NSArray *array = [viewController subviews];
    for (UIView *view in array) {
...
+ (MozTopAlertView*)showWithType:(MozAlertType)type text:(NSString*)text viewController:(UIView*)viewController{
    ...
    [viewController addSubview:alertView];
    [alertView show];
    return alertView;
}
...
- (instancetype)initWithType:(MozAlertType)type text:(NSString*)text doText:(NSString*)doText// viewController:(UIView*)viewController
{
    self = [super init];
    if (self) {
...
- (void)setType:(MozAlertType)type text:(NSString*)text doText:(NSString*)doText{
    ...
    [self setFrame:CGRectMake(([UIScreen mainScreen].bounds.size.width - width)*0.5, -40, width, 50)];
    CGFloat leftIconWidth = 40;
    
    leftIcon = [[UILabel alloc]initWithFrame:CGRectMake(6, 0, leftIconWidth, CGRectGetHeight(self.frame))];
    ...
    switch (type) {
        ...
        case MozAlertTypeWarning:
            ...
            leftIcon.text = [NSString fontAwesomeIconStringForEnum:FAExclamationTriangle];
    ...
    UILabel *textLabel = [[UILabel alloc]initWithFrame:CGRectMake(9, 0, textLabelWidth, CGRectGetHeight(self.frame))];
